...
class LocalMeetingRuntime {
  ...
  public async listDevices(target: WebContents): Promise<Array<{ id: number; name: string; host: string; isDefault: boolean }>> {
    ...
    const script = [
      ...
      "    try: return int(device.get('max_input_channels') or 0)",
      '    except (TypeError, ValueError): return 0',
      'def host_name(device):',
      "    try: return str(host_apis[int(device.get('hostapi', -1))].get('name') or '未知接口')",
      "    except (IndexError, TypeError, ValueError): return '未知接口'",
      'def host_priority(device):',
      '    name = host_name(device).upper()',
      "    if 'WASAPI' in name: return 0",
      "    if 'MME' in name: return 1",
      "    if 'DIRECTSOUND' in name: return 2",
      "    if 'WDM-KS' in name: return 3",
      '    return 4',
      'input_ids = [index for index, device in enumerate(devices) if input_channels(device) > 0]',
      'try:',
      "    raw_default = sd.default.device[0]",
      '    default_id = int(raw_default) if raw_default is not None else -1',
      'except (TypeError, ValueError, IndexError, OverflowError):',
      '    default_id = -1',
      'if default_id not in input_ids:',
      '    host_defaults = []',
      '    for api in host_apis:',
      '        try:',
      "            candidate = int(api.get('default_input_device', -1))",
      '            if candidate in input_ids: host_defaults.append(candidate)',
      '        except (TypeError, ValueError):',
      '            pass',
      '    default_id = min(host_defaults, key=lambda index: host_priority(devices[index])) if host_defaults else -1',
      'if default_id not in input_ids and input_ids:',
      '    default_id = min(input_ids, key=lambda index: host_priority(devices[index]))',
      'items = []',
      'seen = set()',
      'def add_device(index, is_default=False):',
      '    device = devices[index]',
      "    raw_name = str(device.get('name') or '').strip() or '未命名输入设备'",
      "    key = re.sub(r'[^a-z0-9\\u4e00-\\u9fff]+', '', raw_name.lower())",
      '    if not key or key in seen: return',
      '    seen.add(key)',
      "    headset = re.search(r'耳机|耳麦|headset|hands[- ]?free|bluetooth|buds|airpods|airmars', raw_name, re.I)",
      "    label = ('系统默认麦克风 · ' if is_default else '') + raw_name",
      "    if not is_default: label += ' · 耳机麦克风' if headset else ' · 输入设备'",
      "    items.append({'id': index, 'name': label, 'host': host_name(device), 'isDefault': is_default})",
      'if default_id in input_ids: add_device(default_id, True)',
      'for index in sorted(input_ids, key=lambda item: (host_priority(devices[item]), item)):',
      '    if index != default_id: add_device(index, False)',
